...
export default function Dashboard() {
  ...
  const { data: searchResults = [] } = useQuery<WineType[]>({
    queryKey: [`/api/cellars/${cellarId}/wines/search`, searchQuery],
    queryFn: async () => {
      if (!searchQuery.trim() || !cellarId) return [];
      const response = await fetch(`/api/cellars/${cellarId}/wines/search?q=${encodeURIComponent(searchQuery.trim())}`);
      if (!response.ok) throw new Error('Search failed');
      return response.json();
    },
    enabled: searchQuery.trim().length > 0 && !!cellarId,
  });
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <h1 className="text-3xl font-bold">
              {cellar?.name || "Dashboard"}
            </h1>
            <p className="text-wine-600 dark:text-wine-300">
              {cellar?.description || "Overview of your wine collection"}
            </p>
          </div>
          
          <DashboardStats cellarId={cellarId} />
                  ...
                  {searchQuery.trim() && searchResults.length > 0 && (
                    <div className="mb-4 space-y-2 max-h-48 overflow-y-auto">
                      ...
                      {searchResults.length > 5 && (
                        <Link href={`/cellar/${cellarId}/wines`}>
